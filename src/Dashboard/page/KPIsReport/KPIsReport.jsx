...
const KPIsReport = () => {
  ...
  // const monthlyData = useMemo(
  //   () => calculateMonthlyData(filteredLeads),
  //   [filteredLeads],
  // );
...
//     : null;
//   const endDate = range[0].endDate ? range[0].endDate.toISOString() : null;
...
//   // ✅ Default ALL data
//   const { data: allLeads = [], isLoading: leadLoading } = useQuery({
...
//   // ✅ Range based fetch
//   const { data: rangeLeads = [] } = useQuery({
//     queryKey: ["opportunities-range", startDate, endDate],
...
//   const { data: rangeMessages = [] } = useQuery({
//     queryKey: ["messages-range", startDate, endDate],
//     enabled: !!startDate && !!endDate,
//     queryFn: async () => {
//       const { data } = await axiosSecure.get(
//         `/messages?from=${startDate}&to=${endDate}`,
...
//   const effectiveLeads = startDate && endDate ? rangeLeads : allLeads;
//   const effectiveMessages = startDate && endDate ? rangeMessages : allMessages;

//   /* ---------------- Filtered by Clinic ---------------- */
//   const filteredLeads = useMemo(
//     () =>
//       effectiveLeads.filter((l) => selectedClinicPipelineIds.has(l.pipelineId)),
//     [effectiveLeads, selectedClinicPipelineIds],
//   );
...
//   const conversionLead = filteredLeads.filter((l) =>
//     conversationPipelineStageIdSet.has(l.pipelineStageId),
//   );

//   const totalBooked = filteredLeads.filter((l) =>
//     bookingPipelineStageIdSet.has(l.pipelineStageId),
//   );

//   const showingLead = filteredLeads.filter((l) =>
//     showingPipelineStageIdSet.has(l.pipelineStageId),
//   );

//   const closeLead = filteredLeads.filter((l) =>
//     closePipelineStageIdSet.has(l.pipelineStageId),
//   );
...
//     () => (range[0].endDate ? new Date(range[0].endDate) : new Date()),
//     [range],
//   );
...
//       map[key].totalLead++;
//       if (conversationPipelineStageIdSet.has(l.pipelineStageId))
//         map[key].conversion++;
//       if (bookingPipelineStageIdSet.has(l.pipelineStageId)) map[key].booking++;
//       if (showingPipelineStageIdSet.has(l.pipelineStageId)) map[key].showing++;
//       if (closePipelineStageIdSet.has(l.pipelineStageId)) map[key].close++;
//     });
...
//   if (leadLoading || msgLoading || !selectedClinics.length) return <Loading />;
